...
use vm::{CreateContractAddress, EnvInfo};
use machine::{
...
use common_types::{
    ...
    transaction::{CallError, Action, LocalizedTransaction, SignedTransaction, UnverifiedTransaction},
    receipt::{LocalizedReceipt, TransactionOutcome},
    filter::Filter
};
use ethereum_types::{H256, H64, U256, H160};
    ...
    pub fn receipt(&self) -> Fallible<LocalizedReceipt> {
        match self.snapshot.output {
            TxnOutput::Success(ref value) => {
                ...
                Ok(LocalizedReceipt {
                    from: H160::zero(), // FIXME
                    to: Some(H160::zero()), // FIXME
                    transaction_hash,
                    transaction_index,
                    block_hash,
                    block_number,
...
impl mkvs::MKVS for BlockSnapshotMKVS {
